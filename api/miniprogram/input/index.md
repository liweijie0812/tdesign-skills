...
上游文档：https://raw.githubusercontent.com/Tencent/tdesign-miniprogram/develop/packages/components/input/README.md
...
default-value | String / Number | undefined | 输入框的值。非受控属性。TS 类型：`InputValue` `type InputValue = string \| number`。[详细类型定义](./type.ts) | N
...
clear | \- | 清空按钮点击时触发
click | `(trigger: InputTrigger)` | `0.32.0`。点击事件。[详细类型定义](./type.ts)。<br/>`type InputTrigger = 'suffix' \| 'suffix-icon';`<br/>
