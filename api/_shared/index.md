...
`api/_shared/` 只放跨栈语义、端差异入口和组合使用建议，不作为 props、事件、插槽或类型的权威来源。实现时必须回到当前技术栈的 `api/<stack>/<component>/index.md` 和同目录 `type.ts` / `props.ts` / `common.ts` 确认。
...
- 不替代 `decisions/` 的组件选型规则，也不替代 `scenarios/` 的页面级组合方案。
