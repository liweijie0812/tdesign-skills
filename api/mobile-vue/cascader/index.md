...
上游文档：https://raw.githubusercontent.com/Tencent/tdesign-mobile-vue/develop/src/cascader/cascader.md
...
closeBtn | Boolean / Slot / Function | true | 关闭按钮。TS 类型：`boolean \| TNode`。[通用类型定义](../common.ts) | N
header | Slot / Function | - | 头部。TS 类型：`TNode`。[通用类型定义](../common.ts) | N
...
middleContent | Slot / Function | - | `1.9.0`。中间内容。TS 类型：`TNode`。[通用类型定义](../common.ts) | N
...
placeholder | String / Slot / Function | '选择选项' | 未选中时的提示文案。TS 类型：`string \| TNode`。[通用类型定义](../common.ts) | N
...
close | `(trigger: CascaderTriggerSource)` | 关闭时触发。[详细类型定义](./type.ts)。<br/>`type CascaderTriggerSource = 'overlay' \| 'close-btn' \| 'finish'`<br/>
