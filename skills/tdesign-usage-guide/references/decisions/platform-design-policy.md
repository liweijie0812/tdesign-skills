...
- 组件选型优先查 `web/component-usage-map.md`、`web/similar-components.md` 和 `web/when-to-use/`。
...
- 小程序不单独维护设计指南；组件选型与交互口径复用 Mobile，具体 API 和平台能力查 `tdesign-miniprogram/references/api/`。
- 组件选型优先查 `mobile/component-usage-map.md`、`mobile/similar-components.md` 和 `mobile/when-to-use/`，再查通用决策文档。
...
- 出现“小程序”字样时，设计规则仍走 Mobile，但实现必须额外检查微信平台能力和 `tdesign-miniprogram/references/api/`。
