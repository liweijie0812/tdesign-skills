...
适用于 `tdesign-react`、`tdesign-vue-next` 和 `tdesign-vue`。具体 props、事件、插槽和子组件能力必须查当前技术栈 skill 的 `references/api/<component>/index.md`。
...
误判修正：不要只按“是否浮在页面上”判断。危险且需要明确决策的操作不要用 `Drawer` 弱化风险；只需要触发点附近确认的操作不要升级成 `Dialog`；能用 `Select`、`Dropdown`、`Tooltip`、`Popconfirm` 表达的场景不要直接使用底层 `Popup`。
...
误判修正：表单校验和字段错误优先在字段附近表达，不默认使用全局 `Message`；需要用户处理的风险或确认不要只用短暂提示；页面长期可见的规则说明不要用会消失的全局反馈。
...
误判修正：不要因为视觉上是多条记录就默认用 `Table`。只有横向字段对比、排序筛选、分页批量或列级操作重要时才用 `Table`；详情页的键值信息不要用表格模拟；仪表盘区块不要用 `Card` 代替真实数据结构判断。
...
误判修正：`Dropdown` 是操作菜单，不是表单值选择；`Cascader` 强调路径选择，不适合需要同时浏览大量节点状态的树操作；`TreeSelect` 适合把树选择收进表单控件，不适合展示完整目录管理界面。
...
误判修正：不确定总量或时长时不要伪造 `Progress`；首屏列表、卡片、详情结构明确时不要只给空白 `Loading`；短按钮提交反馈优先使用按钮加载态或局部反馈。
