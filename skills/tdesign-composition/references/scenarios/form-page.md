...
- Web 表单页的颜色、间距、圆角和状态样式优先直接写 `var(--td-*)` 形式的 CSS Variables，并参考 ``tdesign-design-spec` 的 `references/design/tokens.md`` 的映射；Mobile / Miniprogram 按当前端 Design Token 和组件变量确认。
...
- Web 自定义样式查 ``tdesign-design-spec` 的 `references/design/tokens.md`` 的常用 Token 速查；交付前按 ``tdesign-design-spec` 的 `references/design/checklist.json`` 的 `outputTemplate` 输出适用项结果。
