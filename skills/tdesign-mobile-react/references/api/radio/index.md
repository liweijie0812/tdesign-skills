...
上游文档：https://raw.githubusercontent.com/Tencent/tdesign-mobile-react/develop/src/radio/radio.md
...
children | TNode | - | 单选内容，同 label。TS 类型：`string \| TNode`。[通用类型定义](../common.ts) | N
content | TNode | - | 单选内容。TS 类型：`string \| TNode`。[通用类型定义](../common.ts) | N
...
keys | Object | - | 用来定义 value / label 在 `options` 中对应的字段别名。TS 类型：`KeysType`。[通用类型定义](../common.ts) | N
name | String | - | HTML 元素原生属性 | N
...
readonly | Boolean | undefined | 只读状态 | N
value | String / Number / Boolean | - | 选中的值。TS 类型：`T` `type RadioValue = string \| number \| boolean`。[详细类型定义](./type.ts) | N
