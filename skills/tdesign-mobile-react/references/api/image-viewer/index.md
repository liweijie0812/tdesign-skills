...
上游文档：https://raw.githubusercontent.com/Tencent/tdesign-mobile-react/develop/src/image-viewer/image-viewer.md
...
deleteBtn | TNode | false | 是否显示删除操作，前提需要开启页码。TS 类型：`boolean \| TNode`。[通用类型定义](../common.ts) | N
images | Array | [] | 图片数组。TS 类型：`Array<string \| ImageInfo>` `interface ImageInfo { url: string; align: 'start' \| 'center' \| 'end' }`。[详细类型定义](./type.ts) | N
