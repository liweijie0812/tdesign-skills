...
上游文档：https://raw.githubusercontent.com/Tencent/tdesign-mobile-react/develop/src/loading/loading.md
...
indicator | TNode | true | 加载指示符，值为 true 显示默认指示符，值为 false 则不显示，也可以自定义指示符。TS 类型：`boolean \| TNode`。[通用类型定义](../common.ts) | N
...
text | TNode | - | 加载提示文案。TS 类型：`string \| TNode`。[通用类型定义](../common.ts) | N
