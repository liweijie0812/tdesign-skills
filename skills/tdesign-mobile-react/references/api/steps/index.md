...
组件简介：用于任务步骤展示或任务进度展示。

上游文档：https://raw.githubusercontent.com/Tencent/tdesign-mobile-react/develop/src/steps/steps.md
...
className | String | - | 类名 | N
style | Object | - | 样式，TS 类型：`React.CSSProperties` | N
children | TNode | - | 步骤描述，同 content。TS 类型：`string \| TNode`。[通用类型定义](../common.ts) | N
content | TNode | '' | 步骤描述。TS 类型：`string \| TNode`。[通用类型定义](../common.ts) | N
extra | TNode | - | 步骤条自定义内容。TS 类型：`string \| TNode`。[通用类型定义](../common.ts) | N
icon | TNode | true | 图标，默认显示内置图标，也可以自定义图标，值为 false 则不显示图标。优先级大于 `status` 定义的图标。TS 类型：`boolean \| TNode`。[通用类型定义](../common.ts) | N
status | String | default | 当前步骤的状态：默认状态（未开始）、进行中状态、完成状态、错误状态。可选项：default/process/finish/error。TS 类型：`StepStatus` `type StepStatus = 'default' \| 'process' \| 'finish' \| 'error'`。[详细类型定义](./type.ts) | N
title | TNode | '' | 标题。TS 类型：`string \| TNode`。[通用类型定义](../common.ts) | N
titleRight | TNode | '' | 标题右侧数据 仅支持 layout = 'vertical' 时。TS 类型：`string \| TNode`。[通用类型定义](../common.ts) | N
