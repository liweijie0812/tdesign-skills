...
组件简介：用于告知用户，该区域的状态变化或者待处理任务的数量。

上游文档：https://raw.githubusercontent.com/Tencent/tdesign-mobile-react/develop/src/badge/badge.md
...
children | TNode | - | 徽标内容，同 content。TS 类型：`string \| TNode`。[通用类型定义](../common.ts) | N
color | String | - | 颜色 | N
content | TNode | - | 徽标内容。TS 类型：`string \| TNode`。[通用类型定义](../common.ts) | N
count | TNode | 0 | 徽标右上角内容。可以是数字，也可以是文字。如：'new'/3/99+。TS 类型：`string \| number \| TNode`。[通用类型定义](../common.ts) | N
