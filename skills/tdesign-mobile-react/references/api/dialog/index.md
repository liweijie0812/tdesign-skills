...
actions | TNode | - | 操作栏。TS 类型：`Array<ButtonProps>`，[Button API Documents](../button/index.md)。[详细类型定义](./type.ts) | N
...
middle | TElement | - | 中间自定义内容。TS 类型：`TNode`。[通用类型定义](../common.ts) | N
...
top | TElement | - | 顶部自定义内容。TS 类型：`TNode`。[通用类型定义](../common.ts) | N
