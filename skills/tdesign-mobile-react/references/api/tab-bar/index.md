...
上游文档：https://raw.githubusercontent.com/Tencent/tdesign-mobile-react/develop/src/tab-bar/tab-bar.md
...
children | TNode | - | 标签栏内容。TS 类型：`TNode`。[通用类型定义](../common.ts) | N
...
className | String | - | 类名 | N
style | Object | - | 样式，TS 类型：`React.CSSProperties` | N
badgeProps | Object | - | 图标右上角提示信息。TS 类型：`BadgeProps`，[Badge API Documents](../badge/index.md)。[详细类型定义](./type.ts) | N
children | TNode | - | 标签内容。TS 类型：`TNode`。[通用类型定义](../common.ts) | N
icon | TElement | - | 图标名称。TS 类型：`TNode`。[通用类型定义](../common.ts) | N
subTabBar | Array | - | 二级菜单。TS 类型：`SubTabBarItem[] ` `interface SubTabBarItem { value: string; label: string }`。[详细类型定义](./type.ts) | N
