...
上游文档：https://raw.githubusercontent.com/Tencent/tdesign-vue/develop/src/input-number/input-number.md
...
value | String / Number | - | 数字输入框的值。当值为 '' 时，输入框显示为空。支持语法糖 `v-model`。TS 类型：`T` `type InputNumberValue = number \| string`。[详细类型定义](./type.ts) | N
