...
2. 简介：声明只服务当前 TDesign 包，并说明本文件仅在完整 collection 模式下作为详细参考。
3. `查阅顺序`：依赖确认 → 覆盖矩阵 → 单组件 API → 类型源 → 示例 → 跨端共享契约 → 页面组合 / 版本回流。
4. `<技术栈> 写法边界`：只写当前栈语法、导入、事件、插槽或平台差异。
5. `约束`：只保留当前栈执行红线、降级策略和路由交接。
