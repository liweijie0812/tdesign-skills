...
上游文档：https://raw.githubusercontent.com/Tencent/tdesign-mobile-vue/develop/src/input/input.md
...
extra | Slot / Function | - | 右侧额外内容。TS 类型：`TNode`。[通用类型定义](../common.ts) | N
...
value | String / Number | - | 输入框的值。支持语法糖 `v-model` 或 `v-model:value`。TS 类型：`InputValue` `type InputValue = string \| number`。[详细类型定义](./type.ts) | N
defaultValue | String / Number | - | 输入框的值。非受控属性。TS 类型：`InputValue` `type InputValue = string \| number`。[详细类型定义](./type.ts) | N
