...
上游文档：https://raw.githubusercontent.com/Tencent/tdesign-mobile-vue/develop/src/tabs/tabs.md
...
名称 | 类型 | 默认值 | 描述 | 必传
-- | -- | -- | -- | --
animation | Object | - | 动画效果设置。其中 duration 表示动画时长。TS 类型：`TabAnimation` `type TabAnimation = { duration: number } & Record<string, any>`。[详细类型定义](./type.ts) | N
bottomLineMode | String | fixed | 激活下划线的模式。可选项：fixed/auto/full | N
list | Array | - | 选项卡列表。TS 类型：`Array<TdTabPanelProps>` | N
middle | Slot / Function | - | 中间内容，介于头部和内容之间。TS 类型：`TNode`。[通用类型定义](../common.ts) | N
...
stickyProps | Object | - | 透传至 Sticky 组件。TS 类型：`StickyProps`，[Sticky API Documents](../sticky/index.md)。[详细类型定义](./type.ts) | N
