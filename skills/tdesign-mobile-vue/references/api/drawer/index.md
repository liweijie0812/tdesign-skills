...
footer | Slot / Function | - | 抽屉的底部。TS 类型：`TNode`。[通用类型定义](../common.ts) | N
...
title | String / Slot / Function | - | 抽屉的标题。TS 类型：`string \| TNode`。[通用类型定义](../common.ts) | N
...
名称 | 参数 | 描述
-- | -- | --
close | `(trigger: DrawerTriggerSource)` | 关闭时触发。。[详细类型定义](./type.ts)。<br/>`type DrawerTriggerSource = 'overlay'`<br/>
