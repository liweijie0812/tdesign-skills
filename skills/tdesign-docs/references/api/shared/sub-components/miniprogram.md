...
- 排版 slots：`Text`、`Title`、`Paragraph` 都可能提供 slots，具体 slot 名称以 `skills/tdesign-miniprogram/references/api/typography/index.md` 为准。
...
- `FormRule` / `FormErrorMessage`：用于表单校验规则和错误信息类型，规则结构以 `skills/tdesign-miniprogram/references/api/form/type.ts` 为准。
