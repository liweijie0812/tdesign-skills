...
- Mobile React API 入口是 `skills/tdesign-mobile-react/references/api/<component>/index.md`，类型源通常是同目录 `type.ts` 和 `skills/tdesign-mobile-react/references/api/common.ts`。
- Mobile Vue API 入口是 `skills/tdesign-mobile-vue/references/api/<component>/index.md`，类型源通常是同目录 `type.ts`、`props.ts` 和 `skills/tdesign-mobile-vue/references/api/common.ts`。
...
- 移动端 `Table` 的列、筛选和自定义单元格优先通过 `columns`、类型和配置项表达；未在当前栈 API 明确出现前，不要生成 `t-table-column`、`TableColumn` 等推测出来的子组件名。
