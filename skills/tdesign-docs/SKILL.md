...
6. Web 视觉一致性、间距、配色、排版、圆角、阴影和状态规则：转到 `tdesign-design-spec` 的 `references/design/visual-consistency-web.md`。未安装时提示补装 `tdesign-design-spec`。
7. 色彩、布局、Token、中后台框架等设计资料：转到 `tdesign-design-spec` 的 `references/design/` 目录（docs 只负责指路，不展开设计细则）。未安装时提示补装 `tdesign-design-spec`。
