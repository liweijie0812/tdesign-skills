...
2. 需要快速参考典型写法时，查 `references/examples/`（示例展示 Vue SFC 模板、easycom 自动引入、@event 事件绑定、v-model 等典型模式）。
3. 组件是否存在先查 `../tdesign-docs/references/meta/stack-matrix-miniprogram.json` 的 `coverage.byStack.uniapp`（uni-app 组件范围与小程序一致）。
...
8. 涉及版本能力时，使用 `tdesign-changelog` 后再回到 UniApp API 文件确认。
