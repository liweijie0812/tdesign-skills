...
组件简介：二维码能够将文本转换生成二维码的组件，支持自定义配色和 Logo 配置。

上游文档：https://raw.githubusercontent.com/Tencent/tdesign-vue-next/develop/packages/components/qrcode/qrcode.md
...
statusRender | Slot / Function | - | 自定义状态渲染器。TS 类型：`(info:StatusRenderInfo) => TNode` `type StatusRenderInfo = {status:QRStatus;onRefresh?: () => void;}`。[通用类型定义](../common.ts)。[详细类型定义](./type.ts) | N
