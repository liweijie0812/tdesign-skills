...
组件简介：用于根据用户输入内容提示更多联想词场景

上游文档：https://raw.githubusercontent.com/Tencent/tdesign-vue-next/develop/packages/components/auto-complete/auto-complete.md
...
default | String / Slot / Function | - | 触发显示联想词下拉框的元素，同 `triggerElement`。TS 类型：`string \| TNode`。[通用类型定义](../common.ts) | N
disabled | Boolean | undefined | 是否禁用 | N
empty | String / Slot / Function | - | 当下拉联想词列表为空时显示的内容。TS 类型：`string \| TNode`。[通用类型定义](../common.ts) | N
...
inputProps | Object | - | 透传 Input 组件全部特性。TS 类型：`InputProps`，[Input API Documents](../input/index.md)。[详细类型定义](./type.ts) | N
...
textareaProps | Object | - | 透传 Textarea 组件全部特性。TS 类型：`TextareaProps`，[Textarea API Documents](../textarea/index.md)。[详细类型定义](./type.ts) | N
tips | String / Slot / Function | - | 输入框下方提示文本，会根据不同的 `status` 呈现不同的样式。TS 类型：`string \| TNode`。[通用类型定义](../common.ts) | N
triggerElement | String / Slot / Function | - | 触发显示联想词下拉框的元素，默认为 Input 组件，可以使用 `trigger` 自定义为 Textarea 组件或其他组件。TS 类型：`string \| TNode`。[通用类型定义](../common.ts) | N
