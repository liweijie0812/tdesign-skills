/**
 * TDesign TDesign React 示例：sticky-tool - compact
 * 覆盖组件：Sticky-tool
 * 来源：组件库源码 packages/components/sticky-tool/_example/compact.tsx
 */

import React from 'react';
import { AddIcon, ChatIcon, QrcodeIcon } from 'tdesign-icons-react';
import { Space, StickyTool } from 'tdesign-react';

import type { StickyToolProps } from 'tdesign-react';

const { StickyItem } = StickyTool;

export default function Compact() {
  const handleClick: StickyToolProps['onClick'] = (context) => {
    console.log('click', context);
  };
  const handleHover: StickyToolProps['onHover'] = (context) => {
    console.log('hover', context);
  };

  return (
    <Space>
      <StickyTool
        style={{ position: 'relative', overflow: 'hidden' }}
        type="compact"
        offset={[-500, -24]}
        onClick={handleClick}
        onHover={handleHover}
      >
        <StickyItem icon={<ChatIcon />} popup="chat"></StickyItem>
        <StickyItem icon={<AddIcon />} popup="add"></StickyItem>
        <StickyItem
          icon={<QrcodeIcon />}
          popup={<img alt="TDesign Logo" width="120" height="120" src="https://tdesign.gtimg.com/site/site.jpg" />}
          popupProps={{ overlayInnerStyle: { padding: '4px', height: '128px' } }}
        ></StickyItem>
      </StickyTool>
    </Space>
  );
}
