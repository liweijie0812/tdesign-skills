...
上游文档：https://raw.githubusercontent.com/Tencent/tdesign-react/develop/packages/components/date-picker/date-picker.md
...
cell | TElement | - | 用于自定义日期单元格的内容。TS 类型：`TNode<{ value: DateValue }>`。[通用类型定义](../common.ts) | N
...
label | TNode | - | 左侧文本。TS 类型：`string \| TNode`。[通用类型定义](../common.ts) | N
...
panelActiveDate | Object | - | 日期选择器中年月下拉框的选中值。TS 类型：`PanelActiveDate` `interface PanelActiveDate {year?: DateValue, month?: DateValue}`。[详细类型定义](./type.ts) | N
...
prefixIcon | TElement | - | 用于自定义组件前置图标。TS 类型：`TNode`。[通用类型定义](../common.ts) | N
presets | TNode | - | 预设快捷日期选择，示例：`{ '元旦': '2021-01-01', '昨天': dayjs().subtract(1, 'day').format('YYYY-MM-DD'), '特定日期': () => ['2021-02-01'] }`。TS 类型：`TNode \| PresetDate` `interface PresetDate { [name: string]: DateValue \| (() => DateValue) }`。[通用类型定义](../common.ts)。[详细类型定义](./type.ts) | N
...
suffixIcon | TElement | - | 用于自定义组件后置图标。TS 类型：`TNode`。[通用类型定义](../common.ts) | N
...
tips | TNode | - | 输入框下方提示文本，会根据不同的 `status` 呈现不同的样式。TS 类型：`string \| TNode`。[通用类型定义](../common.ts) | N
value | String / Number / Array / Date | '' | 选中值。TS 类型：`DateValue \| DateMultipleValue` ` type DateValue = string \| number \| Date ` ` type DateMultipleValue = Array<DateValue> `。[详细类型定义](./type.ts) | N
...
onChange | Function |  | TS 类型：`(value: DateValue, context: { dayjsValue?: Dayjs, trigger?: DatePickerTriggerSource }) => void`<br/>选中值发生变化时触发。[详细类型定义](./type.ts)。<br/>`import { Dayjs } from 'dayjs'`<br/><br/>`type DatePickerTriggerSource = 'confirm' \| 'pick' \| 'enter' \| 'preset' \| 'clear' \| 'tag-remove'`<br/> | N
...
panelActiveDate | Object | - | 日期选择器中年月下拉框的选中值。TS 类型：`PanelActiveDate \| [PanelActiveDate, PanelActiveDate]` `interface PanelActiveDate {year?: DateValue, month?: DateValue}`。[详细类型定义](./type.ts) | N
...
popupProps | Object | - | 透传给 popup 组件的参数。TS 类型：`PopupProps`，[Popup API Documents](../popup/index.md)。[详细类型定义](./type.ts) | N
prefixIcon | TElement | - | 组件前置图标。TS 类型：`TNode`。[通用类型定义](../common.ts) | N
presets | TNode | - | 预设快捷日期选择，示例：{ '特定日期范围': ['2021-01-01', '2022-01-01'], '本月': [dayjs().startOf('month'), dayjs().endOf('month')] }。TS 类型：`TNode \| PresetRange` `interface PresetRange { [range: string]: DateRange \| (() => DateRange)}` `type DateRange = [DateValue, DateValue]`。[通用类型定义](../common.ts)。[详细类型定义](./type.ts) | N
presetsPlacement | String | bottom | 预设面板展示区域（包含确定按钮）。可选项：left/top/right/bottom | N
...
size | String | medium | 输入框尺寸。可选项：small/medium/large。TS 类型：`SizeEnum`。[通用类型定义](../common.ts) | N
status | String | default | 输入框状态。可选项：default/success/warning/error | N
suffixIcon | TElement | - | 组件后置图标。TS 类型：`TNode`。[通用类型定义](../common.ts) | N
timePickerProps | Object | - | 透传 TimePicker 组件属性。TS 类型：`TimePickerProps`，[TimePicker API Documents](../time-picker/index.md)。[详细类型定义](./type.ts) | N
tips | TNode | - | 输入框下方提示文本，会根据不同的 `status` 呈现不同的样式。TS 类型：`string \| TNode`。[通用类型定义](../common.ts) | N
value | Array | [] | 选中值。TS 类型：`DateRangeValue` `type DateRangeValue = Array<DateValue>`。[详细类型定义](./type.ts) | N
