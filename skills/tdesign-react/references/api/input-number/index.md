...
suffix | TNode | - | 后置内容。TS 类型：`string \| TNode`。[通用类型定义](../common.ts) | N
...
value | String / Number | - | 数字输入框的值。当值为 '' 时，输入框显示为空。TS 类型：`T` `type InputNumberValue = number \| string`。[详细类型定义](./type.ts) | N
