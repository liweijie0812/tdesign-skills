---
name: tdesign-react
description: 当项目使用 tdesign-react，或用户提到 TDesign React / Web 组件怎么用、示例代码、props、事件、render、导入方式、JSX 写法或代码落地时使用。
...
2. 组件是否存在：已安装 `tdesign-docs` 时查 `../tdesign-docs/references/meta/stack-matrix-web.json` 的 `coverage.byStack.react`；未安装时以当前栈 `references/api/` 目录实际存在的组件为准，不从记忆推断，建议补装 `tdesign-docs` 获得准确覆盖矩阵。
...
- 不凭经验编造复合组件成员名；例如 `Table` 先查 `columns`、render 和相关类型，不要直接生成 `TableColumn` 或其他未在当前栈 API 中确认的导出名。
- 用户要求“怎么用”或“给个示例”时，先读当前组件 `references/api/<component>/index.md`，必要时补读同栈 examples 和类型源，再组织代码；不要跳过 API 直接给经验写法。
