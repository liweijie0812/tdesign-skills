...
本文按 TDesign Web 组件整理“何时使用”。组件名以 TDesign 为准。若上游设计文档暂未覆盖某个组件，则按企业级后台与业务系统中的通用选型原则补齐，但表述仍保持 TDesign 语境。
...
- 次按钮、文字按钮、虚线按钮或图标按钮用于较低优先级动作。
- 空间有限且操作较多时，将低频操作折叠收纳。
- 高后果操作应使用能表达结果的按钮文案。
...
- 若只是返回顶部，优先使用 `BackTop`；若是视口边缘快捷工具，优先使用 `StickyTool`。
...
- 当提交信息需要校验时使用。
- 字段有明确逻辑顺序时使用纵向布局。
- 表单简单、强调较弱或纵向空间有限时使用横向或行内布局。
- 只读字段值展示优先使用 `Descriptions`。
...
- 当数据需要归纳、分类、比较、关联或差异分析时使用。
- 可与筛选条件搭配，按数据维度过滤。
- 单元格内容需要说明时，可搭配 `Tooltip`。
- 操作列较拥挤时，优先展示高频操作，其余操作折叠收纳。
- 单元格内容过长时，可截断并通过悬浮或展开显示详情。
...
- 用于在悬浮或聚焦时展示简短说明。
- 适合解释图标、截断文本或陌生控件。
- 关键业务信息不应只放在 `Tooltip` 中。

### Tree

- 用于展示和操作层级数据。
- 当用户需要展开、收起、选择、勾选、拖拽或查看节点时使用。
- 紧凑表单中的树形选择使用 `TreeSelect`。
