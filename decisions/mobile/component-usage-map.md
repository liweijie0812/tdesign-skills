...
本文按 TDesign Mobile 设计指南整理移动端和小程序通用“何时使用”。适用于 `tdesign-mobile-react`、`tdesign-mobile-vue` 和 `tdesign-miniprogram` 的设计选型；具体 API、事件、插槽和平台能力仍按当前栈查 `api/mobile-react/`、`api/mobile-vue/` 或 `api/miniprogram/`。
